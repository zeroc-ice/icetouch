...
#if TARGET_OS_IPHONE != 0
void
IceInternal::IncomingConnectionFactory::enterBackground()
{
    IceUtil::Monitor<IceUtil::Mutex>::Lock sync(*this);
    if(_background)
    {
...
void
IceInternal::IncomingConnectionFactory::enterForeground()
{
    IceUtil::Monitor<IceUtil::Mutex>::Lock sync(*this);
    if(!_background)
    {
    ...
    try
    {
        const_cast<AcceptorPtr&>(_acceptor) = _endpoint->acceptor(const_cast<EndpointIPtr&>(_endpoint),
                                                                  _adapter->getName());
        assert(_acceptor);
        _acceptor->listen();
...
IceInternal::IncomingConnectionFactory::initialize(const string& oaName)
{
    ...
    try
    {
        ...
        {
#if TARGET_OS_IPHONE != 0
            _background = IceObjC::registerForBackgroundNotification(this);
            if(_background)
            {
                return; // Don't create the acceptor if we are in the background mode.
            }
#endif
...
IceInternal::IncomingConnectionFactory::setState(State state)
{
    ...
    switch(state)
    {
        ...
        {
#if TARGET_OS_IPHONE != 0
            IceObjC::unregisterForBackgroundNotification(this);
#endif
