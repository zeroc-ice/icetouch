...
EventHandlerWrapper::updateRunLoop()
{
    ...
    else
    {
        ...
        {
            nativeInfo->unregisterFromRunLoop(SocketOperationWrite, false);
        }
        
        if(!(op & (SocketOperationRead | SocketOperationConnect)) || _ready & SocketOperationRead)
        {
            nativeInfo->unregisterFromRunLoop(SocketOperationRead, false);
        }
...
EventHandlerWrapper::ready(SocketOperation op, int error)
{
    if(!_socket)
    {
        ...
        //
        op = StreamNativeInfoPtr::dynamicCast(_handler->getNativeInfo())->unregisterFromRunLoop(op, error != 0);
    }
