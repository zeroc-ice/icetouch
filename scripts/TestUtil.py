...
def run(tests, root = False):
    ...
    for o, a in opts:
        if o == "--continue":
            ...
            global cross
            if a not in ["cpp", "java", "cs", "py", "objc", "rb" ]:
                print("cross must be one of cpp, java, cs, py, objc or rb")
                sys.exit(1)
            cross.append(a)
        elif o == "--all" :
            all = True
...
def getDefaultMapping():
    ...
    for i in range(0, len(here)):
        if here[i] in ["cpp", "cs", "java", "php", "py", "rb", "cppe", "javae", "objc", "tmp"]:
            return here[i]
...
def getCommandLine(exe, config, options = ""):
    ...
    arch = ""
    if isDarwin() and (config.lang == "cpp" or config.lang == "objc"):
        if x64:
            arch = "arch -x86_64 "
        else:
...
def getDefaultServerFile():
    lang = getDefaultMapping()
    if lang in ["rb", "php", "cpp", "cs", "cppe", "objc"]:
        return "server"
    if lang == "py":
        return "Server.py"
...
def getTestEnv(lang, testdir):
    ...
    if isDarwin():
        addPathToEnv("DYLD_LIBRARY_PATH", os.path.join(getIceDir("objc"), "lib"), env)
...
def processCmdLine():
    ...
    for o, a in opts:
        if o == "--ice-home":
            ...
            global cross
            cross.append(a)
            if not a in ["cpp", "java", "cs", "py", "rb", "objc" ]:
                print("cross must be one of cpp, java, cs, py, objc or rb")
                sys.exit(1)
